...
import (
	...
	"xz-go-frame/global"
	context2 "xz-go-frame/model/context"
	req "xz-go-frame/model/entity/commons/request"
	resp "xz-go-frame/model/entity/commons/response"
...
func (api *SysUsersApi) UpdateStatus(c *gin.Context) {
	...
	affected := global.XZ_DB.Unscoped().Model(&sysUser).Where("id = ?", params.Id).Update(params.Filed, params.Value).RowsAffected
	flag := affected > 0
	// 如果保存失败。就返回创建失败的提升
	if !flag {
