...
func (service *BaseService[D, T]) UnGetByID(id D) (data *T, err error) {
	err = global.XZ_DB.Unscoped().Where("id = ?", id).First(&data).Error
	return
}
...
func (service *BaseService[D, T]) DeleteByID(id D) (bool, int64) {
	var data T
	rows := global.XZ_DB.Where("id = ?", id).Delete(&data).RowsAffected
	return rows > 0, rows
}
...
func (service *BaseService[D, T]) UpdateStatus(id D, field string, fieldValue any) (bool, int64) {
	var data T
	affected := global.XZ_DB.Model(data).Where("id = ?", id).Update(field, fieldValue).RowsAffected
	return affected > 0, affected
}
...
func (service *BaseService[D, T]) IncrById(id D, field string) (bool, int64) {
	var data T
	affected := global.XZ_DB.Unscoped().Model(data).Where("id = ? and "+field+" >= 0", id).Update(field, gorm.Expr(field+" + 1")).RowsAffected
	return affected > 0, affected
}
...
func (service *BaseService[D, T]) IncrByIdNum(id D, field string, fieldValue int) (bool, int64) {
	var data T
	affected := global.XZ_DB.Unscoped().Model(data).Where("id = ? and "+field+" >= 0", id).Update(field, gorm.Expr(field+" + ?", fieldValue)).RowsAffected
	return affected > 0, affected
}
...
func (service *BaseService[D, T]) Incrs(ids []D, field string) (bool, int64) {
	var model T
	affected := global.XZ_DB.Unscoped().Model(&model).
