...
import (
	...
	"xz-go-frame/model/entity/video/request"
	"xz-go-frame/model/entity/video/response"
)
...
func (service *VideoService) SaveVideo(xkVideo *video.XkVideo) (err error) {
	err = global.XZ_DB.Model(xkVideo).Create(xkVideo).Error
	return err
}

// 更新
func (service *VideoService) UpdateVideo(xkVideo *video.XkVideo) (err error) {
	err = global.XZ_DB.Model(xkVideo).Updates(xkVideo).Error
	return err
}

// 根据id查询视频信息
func (service *VideoService) GetXkVideoById(id uint) (xkVideo *video.XkVideo, err error) {
	err = global.XZ_DB.Where("id = ?", id).Find(&xkVideo).Error
	return
}
